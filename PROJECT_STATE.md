...
* **MakeCode Static Compilation (Zero Runtime Image Allocations):** Fixed hardware "Panic 020 / Out of Memory" crashes by extracting dynamically generated image templates (``img``` inside functions) to top-level `let` declarations. This prevents redundant RAM consumption (128 bytes per static tile) and leverages MakeCode Arcade's flash ROM mapping. Verified via successful `pxt test` and `pxt build`.
...
* **Save/Load Menu Navigation & Deletion**: Fixed a bug where the load menu was populating slots from unwritten `save_X_name` variables; it now reads from the central save queue (`getSaveList()`). Added full A/B button support to the saving screen (A advances letter position/finalizes save, B moves cursor back/cancels). Implemented save deletion under `Settings > Load` (pressing B on a highlighted save prompts for deletion, where A confirms and B cancels). Added a clean `< BACK` item to the list for unified menu navigation.
* **Save/Load Code Size Optimization**: Extracted bit-packed tile serialization and zombie serialization loops in `save.ts` into shared helper functions (`packWorld`, `unpackWorld`, `packZombies`, `unpackZombies`), deduplicating overworld saving and dungeon transitions. Refactored anonymous controller A and B event callbacks in `player.ts` into named static functions (`handleAPressed`, `handleBPressed`, etc.) to eliminate closure/RefAction runtime wrappers. Simplified Load Game list rendering in `ui.ts`. These modifications reduced compiled binary size by several kilobytes, resolving the `program too big` layout error.

### Remaining Risks

* **Demo AI Memory Footprint:** The autonomous AI script (`demo.ts.bak`) was found to rely heavily on dynamic array reallocation (`push()` and `shift()`) inside continuous ticks to track its pathing history. This causes rapid memory fragmentation and Panic 020 crashes on the Retro Arcade hardware. The demo logic is currently safely removed from compilation. If reintegrated, all pathing arrays must be rewritten as fixed-length circular buffers.
...
```bash
pxt build && cp built/debug/binary.js assets/js/binary.js
```
